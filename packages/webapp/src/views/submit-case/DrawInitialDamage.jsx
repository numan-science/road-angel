...
const DrawInitialDamage = ({ accidentCaseId, setInitialDamageUrl }) => {
  ...
  const handleExport = () => {
    ...
    tempContext.fillStyle = 'rgb(211, 211, 211)';
  ...
  //       tempContext.save(); 
  //       tempContext.translate(image.x - minX, image.y - minY);
  //       tempContext.rotate((image.angle * Math.PI) / 180);
  //       tempContext.drawImage(
  //         img,
  //         -img.width / 2,
  //         -img.height / 2,
  ...
  const handleSave = () => {
    return new Promise((resolve, reject) => {
      ...
      lines.forEach((line) => {
       ...
       const savedWidth = 300;
    const savedHeight = 180;
    ...
    //   resolve(savedDataURL);
      ...
      const dataURL = savedCanvas.toDataURL();
      setInitialDamageUrl(dataURL); // Not sure where this function comes from
      toast.push(
        <Notification className="mb-4" type="success">
          Successfully Save
        </Notification>
      ...
      <div className="bg-white dark:bg-gray-800">
        <Card>
          <h2 className="text-xl font-bold mb-4">
            {t('heading.Create Initial Impact')}
          </h2>
          <Card className="mb-4">
...
export default DrawInitialDamage
