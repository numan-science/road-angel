...
const BusinessAgentList = (props) => {
	...
	const [businessAgentId, setBusinessAgentId] = useState(null);

	const confirmDelete = (businessAgentId) => {
		setConfirmVisible(true);
		setBusinessAgentId(businessAgentId);
	};
	...
	const confirmDialog = () => (
		<ConfirmDialog
			...
			loading={deleting}
		>
			Are you sure you want to delete this business agent?
		</ConfirmDialog>
	);
	...
	const handleDelete = async () => {
		try {
			setDeleting(true);
			const response = await deleteUser(businessAgentId);
			toast.push(
				<Notification className="mb-4" type="success">
					Business Agent Deleted !
				</Notification>
			);
			getTableData();
			setConfirmVisible(false);
			setBusinessAgentId(null);
		} catch (error) {
			toast.push(
	...
	const tableColumns =[
	  ...
	  const canPerformActions = useAuthority(
		["can_edit_business_agents","can_delete_business_agents"],
	);
			...
			Cell: (props) => {
				...
				return (
					<div className="flex justify-start text-lg">
						<AuthorityCheck authority={["can_edit_business_agents"]}>
							<span className="cursor-pointer p-2 hover:text-blue-500">
								<HiOutlinePencil onClick={() => handleEditClick(row)} />
							</span>
						</AuthorityCheck>

						<AuthorityCheck authority={["can_delete_business_agents"]}>
...
export default BusinessAgentList;
